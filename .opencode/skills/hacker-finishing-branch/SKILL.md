...
> **Texto unico.** Comandos de verificacao e path da PR vem do perfil do repo
...
**Anuncio de inicio**: "Estou usando a skill hacker-finishing-branch para finalizar o desenvolvimento."
...
- Retornar para `hacker-subagent-driven-development`
- Reportar qual comando falhou e por que
- NAO prosseguir

### Step 1.5: GATE de integridade do harness (lei F10)

Se o `git diff` da entrega toca qualquer arquivo de harness (`.opencode/`): executar o
procedimento de espelhamento de `.opencode/rules/hacker-harness-integrity.md`. Resultado
precisa ser **HARNESS INTEGRO** antes de gerar a PR. Deriva detectada = reconciliar via
`hacker-harness-sync` (com o HARD-GATE dela) antes de continuar. Se o diff nao toca
...
- [x] Postura de rede: verificar-vazamento.sh GOOD
- [x] Gate F10 (harness): [HARNESS INTEGRO | nao se aplica]

## Beneficios
[Reuso, seguranca, manutenibilidade]
...
> comandos de analise como `git diff`, `git log`, `git status` ou similares.

| Command | Result (OK/FAIL) | Observations |
|---------|------------------|--------------|
| bash -n <script> | OK | [observacao] |
| shellcheck <script> | OK | [observacao] |
| py_compile <arquivo> | OK | [observacao] |
| docker compose config | OK | [observacao] |
| git diff --check | OK | [observacao] |
| verificar-vazamento.sh | OK | GOOD |
```

> **IMPORTANTE**: O modelo acima e a ESTRUTURA. Substitua todos os valores pelos dados
> reais do seu git diff e das saidas literais desta sessao.
...
Apos aprovacao, salvar em `.opencode/pr/`, nome `PR_NNN_nome-descritivo.md`.
...
- Mudanca em harness exige harness integro ANTES da PR (F10)
- Suite de verificacao do perfil: bash -n + shellcheck + py_compile + docker config + git diff --check
- PR segue convencoes deste projeto (PT-BR, paths exatos, evidencias reais)
- Regra canonica anti-invencao: `.opencode/rules/hacker-epistemic-safety.md`. Evidencias da PR
  vem do git diff/log real (F3), nunca fabricadas.
- Fernando faz commit/push manualmente - skill NAO faz git write

## Integracao

**Skill anterior**: `hacker-doc-sync` (4.6) ou diretamente a PARADA UNICA
**Final skill no pipeline Hacker Etico SDD**
